...
chatRouter.post('/start', async (c) => {
  try {
    ...
    return c.json({ success: false, error: 'Failed to start chat session' }, 500);
  }
});
...
chatRouter.post('/message', async (c) => {
  try {
    ...
    if (!geminiService) {
      return c.json({ success: false, error: 'Chat session not initialized. Please start a chat first.' }, 400);
    }
