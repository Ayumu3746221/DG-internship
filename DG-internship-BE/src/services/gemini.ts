...
export class GeminiService {
  ...
  async initializeChat() {
    ...
    ※相関関係や傾向を見つけてください。
    ※単位が記載されていない場合、日本円（JPY）での売上やLTVの数値を使用しています。
    ...
    this.chatSession = this.model.startChat({
      generationConfig: {
        ...
        topK: 1,
        topP: 1,
        maxOutputTokens: 4096, // 最大出力トークン数を増加（より詳細な分析のため）
      },
      history: [
        {
          role: 'user',
  ...
  async getInitialAnalysis(): Promise<string> {
    ...
    **4. 今後の運営戦略に向けた具体的な提案**
    ...
    ※直接レポートを生成してください。冒頭に「以下は」などの前置きは不要です。但是要一个： ＃＃ 横断的データ分析レポート
    ※初心者にも分かりやすい表現で、専門用語は避けてください。
    ※各項目には実際のデータに基づく具体的な数値や傾向を必ず含めてください。
    ※1～4の各項目のタイトルが必ず含まれるようにしてください。（括弧は、適切省略して）
    ※1～4の各項目はタイトル（h1,h2,h3,h4,h5）形式じゃなくて、普通の太文字で充分!!!必ず＃＃＃は要らない
