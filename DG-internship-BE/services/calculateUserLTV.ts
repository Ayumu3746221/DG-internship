 ...
 */
function calculateAPV(transactions: Transaction[]): number {
 ...
 * 月間購入頻度を計算する（修正版）
 */
function calculateMonthlyFrequency(
 ...
 */
export function calculateUserLTV(
  userTransactions: Transaction[],
  snapshotDate: Date
): number {
  // 基準日までの取引履歴にフィルタリング
  const transactionsUpToDate = userTransactions.filter(
    (t) => t.date <= snapshotDate
  );
  ...
  const monthlyFrequency = calculateMonthlyFrequency(
