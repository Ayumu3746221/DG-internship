...
debug.get("/fetch-data/:appId", async (c) => {
    try {
        ...
        const result = Array.from(userHistoryMap.entries()).map(([userId, transactions]) => ({
            userId,
        ...
        return c.json({
            ...
            data: {
                ...
                totalTransactions: Array.from(userHistoryMap.values()).reduce((sum, transactions) => sum + transactions.length, 0),
...
debug.get("/ltv-batch/:appId", async (c) => {
    try {
        ...
        const avgHigh = chartData.reduce((sum, day) => sum + day.high, 0) / totalDays || 0;
        const avgMiddle = chartData.reduce((sum, day) => sum + day.middle, 0) / totalDays || 0;
        const avgLow = chartData.reduce((sum, day) => sum + day.low, 0) / totalDays || 0;
        return c.json({
            success: true,
            data: {
                appId,
